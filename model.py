...
def softmax_cross_entropy_backward(logits, y):
    ...
    # Gradient of mean softmax cross-entropy.
    dlogits = (probs - targets) / logits.shape[0]

    # Normalize exact negative zeros to positive zeros.
    dlogits[dlogits == 0.0] = 0.0
...
def init_conv_layer(out_channels, in_channels, kernel_size, seed=0):
    # Each output unit sees all input channels across the kernel area.
...
def forward_conv_block(x, W, b, pool_size, stride, pad):
    ...
    pool_out, pool_cache = maxpool2d_forward(
        relu_out,
        kernel=pool_size,
        stride=pool_size
    )
...
def iterate_minibatches(x, y, batch_size, seed=0):
    # Generate one shuffled order shared by features and labels.
    indices = shuffle_indices(x.shape[0], seed=seed)

    # Yield only complete batches, dropping any trailing partial batch.
...
def train_one_epoch(
    ...
    # Store the loss from each complete minibatch.
    losses = []

    # Iterate through one shuffled pass over the training data.
    for xb, yb in iterate_minibatches(
        ...
        # Advance the Adam step counter once per minibatch.
        step_counter += 1

        # Perform one complete forward/backward/update step.
